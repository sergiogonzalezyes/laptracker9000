...
export default function LapTimeChart({ laps, height = 150, accentColor = 'var(--accent)' }: Props) {
  const [hovered, setHovered] = useState<LapPoint & { cx: number; cy: number } | null>(null);
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        {/* Y axis labels */}
        {yTicks.filter((_, i) => i % 2 === 1 || i === 0 || i === 3).map((ms, i) => (
          <text key={i}
        ...
        <line
          x1={PAD.left} y1={bestY.toFixed(1)}
          x2={PAD.left + chartW} y2={bestY.toFixed(1)}
          stroke="var(--green)" strokeWidth="1"
          strokeDasharray="6 4" opacity="0.3"
        />

        {/* Area fill */}
        <path d={areaPath} fill="rgba(255,255,255,0.025)" />
        ...
        {pts.map((p, i) => {
          ...
          return (
            <circle key={i}
              ...
              r={isHov ? 6 : isBest ? 5 : 3.5}
              fill={isBest ? 'var(--green)' : isHov ? 'var(--text-primary)' : '#333'}
              stroke={isBest ? 'var(--green)' : '#111'}
              strokeWidth="1.5"
            />
          );
        })}
        ...
        {pts.filter((_, i) =>
          ...
          <text key={i}
            x={p.cx.toFixed(1)} y={PAD.top + chartH + 20}
      ...
      {hovered && (
        <div style={{
          ...
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 14, fontWeight: 700, color: hovered.lapTimeMs === minMs ? 'var(--green)' : 'var(--text-primary)' }}>
            {formatLapTime(hovered.lapTimeMs)}
          </div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, display: 'flex', gap: 8 }}>
            <span>Lap {hovered.lapNumber}</span>
            {hovered.lapTimeMs === minMs && <span style={{ color: 'var(--green)' }}>Best</span>}
            {hovered.isPB && <span style={{ color: 'var(--green)' }}>PB</span>}
          </div>
        </div>
      )}
    </div>
