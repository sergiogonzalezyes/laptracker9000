import { ParsedLap, ParsedSession, ParseResult } from './types';
...
interface PendingSplits {
  split0Ms?: number;
  split1Ms?: number;
}

interface ActiveLapBlock {
  driverName: string;
  lapTimeMs: number;
  cuts: number;
  lapNumber: number;
...
interface ParserState {
  ...
  inPickup: boolean;       // after NEW PICKUP CONNECTION
  afterVersion: boolean;   // next non-empty line after VERSION 202 = driver name
  pendingDriverName: string | null;
  pendingDriverGuid: string | null;
  pendingDriverSlot: number | null;

  slots: Map<number, DriverSlot>;
  pendingSplits: Map<number, PendingSplits>;

  activeLapBlock: Partial<ActiveLapBlock> | null;
  inLapBlock: boolean;

  currentSessionLaps: ParsedLap[];
  completedSessions: ParsedSession[];
}

function newState(serverName: string): ParserState {
  const now = new Date();
  return {
    serverName,
    track: '',
...
function finalizeSession(state: ParserState, endedAt: Date | null = null): void {
...
export function processLine(line: string, state: ParserState): void {
  ...
  if (splitMatch) {
    ...
    if (splitIdx === 0) splits.split0Ms = ms;
    else if (splitIdx === 1) splits.split1Ms = ms;
  ...
  if (trimmed === 'Car.onLapCompleted END') {
    if (state.activeLapBlock?.driverName && state.activeLapBlock.lapTimeMs !== undefined) {
      const block = state.activeLapBlock as ActiveLapBlock;
  ...
  if (discMatch) {
    ...
    for (const [slot, info] of state.slots) {
      if (info.name === name) { state.slots.delete(slot); break; }
    }
    return;
  }
}

export function parseLines(lines: string[], state: ParserState): void {
  for (const line of lines) processLine(line, state);
}

export function parseLogContent(content: string, serverName: string): ParseResult {
...
export function createParserState(serverName: string): ParserState {
