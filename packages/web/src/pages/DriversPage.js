...
export default function DriversPage() {
    ...
    if (drivers.length === 0)
        return (_jsx("div", { style: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#333', fontSize: 13 }, children: "No drivers yet \u2014 race first and claim your profile." }));
    const totalPages = Math.ceil(drivers.length / PER_PAGE);
    const pageDrivers = drivers.slice(page * PER_PAGE, (page + 1) * PER_PAGE);
    return (_jsxs("div", { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 12 }, children: [_jsx("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(220px, 100%), 1fr))', gap: 10 }, children: pageDrivers.map(d => (_jsx(Link, { to: `/drivers/${encodeURIComponent(d.name)}`, style: { textDecoration: 'none' }, children: _jsxs("div", { style: {
                            ...
                            el.style.borderLeftColor = d.color || '#cc0000';
                                ...
                                ].map(s => (_jsxs("div", { style: { flex: 1, padding: '6px 8px', background: '#0a0a0a', border: '1px solid #181818', textAlign: 'center' }, children: [_jsx("div", { style: { fontSize: 8, color: 'var(--text-muted)', fontFamily: 'var(--font-display)', letterSpacing: '0.1em', marginBottom: 3 }, children: s.label }), _jsx("div", { style: { fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 12, color: 'var(--chrome-light)' }, children: s.value })] }, s.label))) })] }) }, d.name))) }), totalPages > 1 && (_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }, children: [_jsx("button", { onClick: () => setPage(p => Math.max(0, p - 1)), disabled: page === 0, style: { padding: '5px 12px', fontSize: 12, fontWeight: 600, background: 'var(--bg-elevated)', color: page === 0 ? 'var(--text-muted)' : 'var(--text-primary)', border: '1px solid var(--border)', borderRadius: 6, cursor: page === 0 ? 'default' : 'pointer' }, children: "\u2190 Prev" }), Array.from({ length: totalPages }, (_, i) => (_jsx("button", { onClick: () => setPage(i), style: { width: 28, height: 28, fontSize: 12, fontWeight: 600, background: i === page ? 'var(--accent)' : 'var(--bg-elevated)', color: i === page ? '#fff' : 'var(--text-muted)', border: `1px solid ${i === page ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 6, cursor: 'pointer' }, children: i + 1 }, i))), _jsx("button", { onClick: () => setPage(p => Math.min(totalPages - 1, p + 1)), disabled: page === totalPages - 1, style: { padding: '5px 12px', fontSize: 12, fontWeight: 600, background: 'var(--bg-elevated)', color: page === totalPages - 1 ? 'var(--text-muted)' : 'var(--text-primary)', border: '1px solid var(--border)', borderRadius: 6, cursor: page === totalPages - 1 ? 'default' : 'pointer' }, children: "Next \u2192" })] }))] }));
}
